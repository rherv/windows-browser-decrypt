...
pub fn read_local_state(file_path: impl AsRef<Path>) -> Result<Vec<u8>, ExporterError> {
    ...
    let encoded_key = obj["os_crypt"]["encrypted_key"].as_str().ok_or(
        ExporterError::IO("Failed to get encoded key from Local State".to_string())
    )?;

    let mut decoded_key = general_purpose::STANDARD.decode(encoded_key).map_err(|e| {
        ExporterError::IO(e.to_string())
    })?;
